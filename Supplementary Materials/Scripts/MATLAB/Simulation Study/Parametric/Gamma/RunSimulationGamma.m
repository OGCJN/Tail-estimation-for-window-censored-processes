%% Simulation study for the full-parametric Gamma model.
...
for i=1:numel(Ns)
    for j=1:numel(sigmas)
        ...
        parfor p=1:m
            [S,L]=SimulateGlancesGamma(N,k,sigma,mu1,w);                             % Simulation step
            [k_hat(p), sigma_hat(p), CI_k(:,p), CI_sigma(:,p)]=EstimateGamma(S,L,w); % Estimation step
        end
